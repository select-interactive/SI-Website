...
/* si.contact */
(function(m,a){window.si=window.si||{};si.contact=function(){function e(){var c=a("#form-contact"),e=a("#contact-opts").find(":input:checkbox"),h=a("#tb-name"),i=a("#tb-email"),j=a("#tb-phone"),k=a("#ta-message"),l=a("#btn-submit"),d=a("#status-contact");l.on("click",function(f){f.preventDefault();var g=!1,b="";e.each(function(){this.checked&&(""!==b&&(b+="&nbsp;&nbsp;"),b+=this.value)});""===b&&(g=!0);c.find(":input:text, textarea").each(function(){0===a.trim(this.value).length?(this.className="error", g=!0):this.className=""});g?(c.find(".error").first().focus(),d.html('<p class="status-error">'+(""===b?"You must select a contact reason. ":"")+"All fields are required.</p>")):(c.find("input, textarea, button").attr("disabled","disabled"),d.html('<p class="status-working">Submitting form...&nbsp;'+si.loadingGif+"</p>"),f=JSON.stringify({reason:b,name:a.trim(h.val()),email:a.trim(i.val()),phone:a.trim(j.val()),message:a.trim(k.val())}),si.ajax("/webservices/wsContact.asmx/sendContactMessage",f,function(a){a&& a.d?(a=a.d,"success"===a.toLowerCase()?c.html('<p class="nm">Thank you for contacting us. A member of the Select Interactive team will be in touch with you as quickly as possible!</p>'):(c.find("input, textarea, button").removeAttr("disabled"),d.html('<p class="status-error">'+a+"</p>"))):(c.find("input, textarea, button").removeAttr("disabled"),d.html('<p class="status-error">An error has occured. Please try again or reload the page.</p>'))},function(){c.find("input, textarea, button").removeAttr("disabled"); d.html('<p class="status-error">An error has occured. Please try again or reload the page.</p>')}))})}a(function(){e()})}()})(document,jQuery);