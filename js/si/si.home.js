...
(function( document, $ ) {
    ...
    function init() {
        // transition in the page elements
        $( 'header, nav, #content-wrapper, footer' ).addClass( 'in' );
    ...
    function initCycle() {
        var _cycle = $( '#cycle' ),
            _overlay = $( '#cycle-overlay' ),
            _arrows = _cycle.find( '.arrow' ),
            _text = $( '#specialize-text' ),
            _imgs = _cycle.find( 'img' ),
            _curImg = _imgs.filter( '.in' ),
            rotateTimeout = null,
            index = 0,
            csstransitions = Modernizr.csstransitions;
                
        //_overlay.addClass( _curImg.data( 'class' ) );
        _text.html( _curImg.data( 'text' ) );

        !csstransitions && $( '.arrow' ).fadeTo( 0, 0.7 );

        function rotate( isBack ) {
            var oldClass = _curImg.data( 'class' )
            
            if ( csstransitions ) {
                _curImg.removeClass( 'in' );
                _text.removeClass( 'in' );
            }
            else {
                _curImg.fadeOut( 1000 );
            }
                
            isBack 
                ? --index < 0 && ( index = _imgs.length - 1 )
                : ++index === _imgs.length && ( index = 0 );

            _curImg = $( _imgs[index] );
                
            csstransitions ? _curImg.addClass( 'in' ) : _curImg.fadeIn( 1000 );
                
            setTimeout(function() { 
                _text.html( _curImg.data( 'text' ) );
                csstransitions && _text.addClass( 'in' );
            }, 475 );
                
            rotateTimeout = setTimeout( rotate, 4000 );
        }
        
        _arrows
            .on( 'mouseenter', function() {
                !csstransitions && $( this ).fadeTo( 300, 0.95 );
            })
            .on( 'mouseleave', function() {
                !csstransitions && $( this ).fadeTo( 300, 0.7 );
            })
            .on( 'click', function( e ) {
                // prevent postback
                e.preventDefault();
